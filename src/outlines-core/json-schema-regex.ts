...
class Parser {
  ...
  toRegex(json: any): string {
    if (typeof json !== 'object' || json === null) {
      throw new JsonSchemaError('Expected JSON object', 'UnsupportedJsonSchema')
    }
    ...
    throw new JsonSchemaError(`Unsupported JSON Schema structure: ${JSON.stringify(json)}`, 'UnsupportedJsonSchema')
  }
  ...
  private parseProperties(obj: any): string {
    ...
    if (!properties || typeof properties !== 'object') {
      throw new JsonSchemaError("'properties' not found or not an object", 'PropertiesNotFound')
    }

    const requiredProperties = Array.isArray(obj.required) ? obj.required : []
    const propertyNames = Object.keys(properties)
    const isRequired = propertyNames.map((name) => requiredProperties.includes(name))

    if (isRequired.some(Boolean)) {
      const lastRequiredPos = isRequired.reduce((lastIndex, isReq, index) => (isReq ? index : lastIndex), -1)

      for (let i = 0; i < propertyNames.length; i++) {
        const name = propertyNames[i]
        const value = properties[name]

        let subregex = `${this.whitespacePattern}"${this.escapeRegex(name)}"${this.whitespacePattern}:${this.whitespacePattern}`

        try {
          subregex += this.toRegex(value)
        } catch (e) {
          if (e instanceof JsonSchemaError && e.errorType === 'RefRecursionLimitReached') {
            continue
          }
          throw e
        }
  ...
  private parseAllOf(obj: any): string {
    ...
    if (!Array.isArray(allOf)) {
      throw new JsonSchemaError("'allOf' must be an array", 'AllOfMustBeAnArray')
    }
  ...
  private parseAnyOf(obj: any): string {
    ...
    if (!Array.isArray(anyOf)) {
      throw new JsonSchemaError("'anyOf' must be an array", 'AnyOfMustBeAnArray')
    }
  ...
  private parseOneOf(obj: any): string {
    ...
    if (!Array.isArray(oneOf)) {
      throw new JsonSchemaError("'oneOf' must be an array", 'OneOfMustBeAnArray')
    }
  ...
  private parsePrefixItems(obj: any): string {
    ...
    if (!Array.isArray(prefixItems)) {
      throw new JsonSchemaError("'prefixItems' must be an array", 'PrefixItemsMustBeAnArray')
    }
  ...
  private parseConst(obj: any): string {
    if (!('const' in obj)) {
      throw new JsonSchemaError("'const' key not found in object", 'ConstKeyNotFound')
    }
  ...
  private parseConstValue(value: any): string {
    ...
    if (value && typeof value === 'object') {
      const innerRegex = Object.entries(value)
        .map(([key, val]) => `"${key}"${this.whitespacePattern}:${this.whitespacePattern}${this.parseConstValue(val)}`)
        .join(`${this.whitespacePattern},${this.whitespacePattern}`)
      return `\\{${this.whitespacePattern}${innerRegex}${this.whitespacePattern}\\}`
    }
  ...
  private parseRef(obj: any): string {
    if (this.recursionDepth > this.maxRecursionDepth) {
      throw new JsonSchemaError(`Ref recursion limit reached: ${this.maxRecursionDepth}`, 'RefRecursionLimitReached')
    }
    ...
    try {
      ...
      if (typeof refPath !== 'string') {
        throw new JsonSchemaError("'$ref' must be a string", 'RefMustBeAString')
      }
      ...
      if (parts.length === 2) {
        ...
        throw new JsonSchemaError(`External references are not supported: ${refPath}`, 'ExternalReferencesNotSupported')
      }

      throw new JsonSchemaError(`Invalid reference format: ${refPath}`, 'InvalidReferenceFormat')
    } finally {
      this.recursionDepth--
    }
  }
    ...
    if (Array.isArray(type)) {
      const xorPatterns = type.map((instanceType) => {
        if (typeof instanceType !== 'string') {
          throw new JsonSchemaError("'type' must be a string or an array of string", 'TypeMustBeAStringOrArray')
        }
  ...
  private parseTypeString(instanceType: string, obj: any): string {
    switch (instanceType) {
      ...
      default:
        throw new JsonSchemaError(`Unsupported type: ${instanceType}`, 'UnsupportedType')
    }
  }
  ...
  private parseStringType(obj: any): string {
    ...
    if ('maxLength' in obj || 'minLength' in obj) {
      ...
      if (typeof minLength === 'number' && typeof maxLength === 'number' && minLength > maxLength) {
        throw new JsonSchemaError('maxLength must be greater than or equal to minLength', 'MaxBoundError')
      }

      const formattedMax = typeof maxLength === 'number' ? maxLength.toString() : ''
      const formattedMin = typeof minLength === 'number' ? minLength.toString() : '0'
    ...
    if ('format' in obj) {
      ...
      switch (format) {
        ...
        default:
          throw new JsonSchemaError(`Format ${format} is not supported by Outlines`, 'StringTypeUnsupportedFormat')
      }
    }
  ...
  private parseNumberType(obj: any): string {
    ...
    if (hasBounds) {
      ...
      const integersQuantifier = this.buildQuantifier(minDigitsInteger, maxDigitsInteger, 1, '*')
      const fractionQuantifier = this.buildQuantifier(minDigitsFraction, maxDigitsFraction, 0, '+')
      const exponentQuantifier = this.buildQuantifier(minDigitsExponent, maxDigitsExponent, 0, '+')
  ...
  private parseIntegerType(obj: any): string {
    if ('minDigits' in obj || 'maxDigits' in obj) {
      const [minDigits, maxDigits] = this.validateQuantifiers(obj.minDigits, obj.maxDigits, 1)
  ...
  private parseObjectType(obj: any): string {
    ...
    if (additionalProperties === undefined || additionalProperties === true) {
      const legalTypes: any[] = [{ type: 'string' }, { type: 'number' }, { type: 'boolean' }, { type: 'null' }]
  ...
  private parseArrayType(obj: any): string {
    const numRepeats = this.getNumItemsPattern(obj.minItems, obj.maxItems) || ''
  ...
  private resolveLocalRef(schema: any, pathParts: string[]): any {
    ...
    for (const part of pathParts) {
      if (!(part in current)) {
        throw new JsonSchemaError(`Invalid reference path: ${part}`, 'InvalidReferencePath')
      }
  ...
  private validateQuantifiers(
    ...
    const adjustedMin = typeof minBound === 'number' ? Math.max(0, minBound - startOffset) : null
    const adjustedMax = typeof maxBound === 'number' ? Math.max(0, maxBound - startOffset) : null
  ...
  private buildQuantifier(min: number | null, max: number | null, defaultMin: number, unboundedSuffix: string): string {
  ...
  private getNumItemsPattern(minItems?: number, maxItems?: number): string | null {
